...
function PageBanner({ farmerCount }) {
  return (
...
function SkeletonCard() {
  return (
    <div className="bg-card border border-border rounded-2xl p-5 flex flex-col items-center gap-3 animate-pulse">
...
export default function FarmersPage() {
    ...
    try {
      if (user.role === "farmer") {
        ...
        const res = await fetch("/api/messages/requests", {
          ...
          body: JSON.stringify({ farmerId: farmer._id }),
        });
        const data = await res.json();
  ...
  return (
    <div className="min-h-screen bg-background">
        <Header/>
      {/* ── Full-bleed banner ── */}
      <PageBanner farmerCount={farmers.length} />
