...
import {
  RefreshCw, MapPin, Leaf, Sprout,
  ChevronLeft, ChevronRight, Search,
  Clock, CheckCircle, EyeOff, XCircle, Trash2,
} from "lucide-react";
...
const STATUS_CFG = {
  ...
  approved: { color: B.primary, bg: B.muted,   border: B.border,  icon: CheckCircle },
...
export default function AdminCropsPage() {
  ...
  const loadCrops = async (page = 1) => {
    try {
      ...
      const res  = await fetch(`/api/admin/crops?page=${page}&limit=${pagination.limit}`, { headers: { "x-role": "admin" } });
  ...
  useEffect(() => { loadCrops(1); }, []);

  const updateStatus = async (id, status) => {
    const result = await MySwal.fire({
      title: "Update Crop Visibility?", text: `This crop will be set to "${status}".`,
      icon: "question", showCancelButton: true,
      confirmButtonColor: B.primary, cancelButtonColor: B.accent,
      confirmButtonText: "Yes, update status", background: "#fff",
    });
    if (!result.isConfirmed) return;
    try {
      const res = await fetch(`/api/admin/crops/${id}/status`, { method: "PATCH", headers: { "Content-Type": "application/json", "x-role": "admin" }, body: JSON.stringify({ status }) });
      if (!res.ok) throw new Error("Status update failed");
      MySwal.fire({ toast: true, position: "top-end", icon: "success", title: "Status Updated", showConfirmButton: false, timer: 2000 });
  ...
  const deleteCrop = async (id) => {
    const result = await MySwal.fire({
      title: "Delete Crop Permanently?", text: "This listing will be removed from the marketplace forever.",
      icon: "warning", showCancelButton: true,
      confirmButtonColor: "#C62828", cancelButtonColor: B.accent,
      confirmButtonText: "Yes, delete it", background: "#fff",
    });
    if (!result.isConfirmed) return;
    try {
      const res = await fetch(`/api/admin/crops/${id}`, { method: "DELETE", headers: { "x-role": "admin" } });
      if (!res.ok) throw new Error("Deletion failed");
      MySwal.fire({ toast: true, position: "top-end", icon: "success", title: "Crop Deleted", showConfirmButton: false, timer: 2000 });
      loadCrops(pagination.page);
    } catch (e) { MySwal.fire("Failed", e.message, "error"); }
  };
    ...
    <div style={{ fontFamily: "'DM Sans', sans-serif", minHeight: "100vh", padding: "8px 0" }}>
      ...
      <div style={{ maxWidth: 1100, margin: "0 auto", display: "flex", flexDirection: "column", gap: 20 }}>
        ...
        <header className="header-wrap" style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 14, animation: "fadeUp 0.4s ease both" }}>
          <div>
            <div style={{ display: "inline-flex", alignItems: "center", gap: 7, padding: "4px 12px", borderRadius: 20, background: B.muted, border: `1.5px solid ${B.border}`, marginBottom: 10 }}>
              ...
              <span style={{ color: B.primary, fontSize: 10, fontWeight: 800, letterSpacing: "0.18em", textTransform: "uppercase" }}>Admin · Crops</span>
            </div>
            ...
            <p style={{ color: B.mutedFg, fontSize: 13, fontWeight: 500, margin: "8px 0 0" }}>
          ...
          <div className="header-actions" style={{ display: "flex", alignItems: "center", gap: 8, flexShrink: 0 }}>
            ...
            <button onClick={() => loadCrops(pagination.page)} style={{
        ...
        <div className="pills-wrap" style={{ display: "flex", gap: 8, flexWrap: "nowrap", overflowX: "auto", paddingBottom: 2, animation: "fadeUp 0.45s ease both" }}>
          {[
            { label: "Total",    value: pagination.total || crops.length, color: B.primary,  bg: B.muted,   border: B.border  },
            { label: "Approved", value: counts.approved,                  color: B.primary,  bg: B.muted,   border: B.border  },
            { label: "Pending",  value: counts.pending,                   color: "#E65100",  bg: "#FBE9E7", border: "#FFCCBC" },
            { label: "Hidden",   value: counts.hidden,                    color: "#6A1B9A",  bg: "#F3E5F5", border: "#E1BEE7" },
            { label: "Rejected", value: counts.rejected,                  color: "#C62828",  bg: "#FFEBEE", border: "#FFCDD2" },
          ].map((p) => (
        ...
        <div className="desktop-table" style={{ background: B.card, border: `1.5px solid ${B.border}`, borderRadius: 20, overflow: "hidden", boxShadow: "0 2px 16px rgba(46,125,50,0.08)", animation: "fadeUp 0.5s ease 0.1s both" }}>
            ...
            <table style={{ width: "100%", borderCollapse: "collapse", minWidth: 640 }}>
              <thead>
                <tr style={{ background: B.primary }}>
                  {[{ label: "Crop Details", align: "left" }, { label: "Location", align: "left" }, { label: "Price", align: "center" }, { label: "Visibility", align: "center" }, { label: "Actions", align: "right" }].map((h) => (
              ...
              <tbody>
                {!loading && filtered.map((crop, i) => {
                  const cfg = STATUS_CFG[crop.status] || STATUS_CFG.pending;
                  const StatusIcon = cfg.icon;
                  return (
                    <tr key={crop._id} className="crop-row" style={{ borderBottom: `1px solid ${B.border}`, background: i % 2 === 0 ? "#fff" : "#fafffe" }}>
                      <td style={{ padding: "14px 22px" }}>
                        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                          <div style={{ padding: "9px 10px", borderRadius: 12, background: B.muted, border: `1.5px solid ${B.border}`, flexShrink: 0 }}><Sprout size={16} style={{ color: B.primary }} /></div>
                          <div>
                            <div style={{ fontWeight: 800, fontSize: 14, color: B.foreground }}>{crop.title}</div>
                            <div style={{ fontSize: 10, fontWeight: 900, color: B.primary, textTransform: "uppercase", letterSpacing: "0.1em", marginTop: 3, background: B.muted, border: `1px solid ${B.border}`, borderRadius: 6, padding: "1px 7px", display: "inline-block" }}>{crop.cropType}</div>
                          </div>
                        </div>
                      </td>
                      <td style={{ padding: "14px 22px" }}>
                        <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                          <div style={{ padding: "5px 6px", borderRadius: 8, background: "#FBE9E7", border: "1px solid #FFCCBC" }}><MapPin size={11} style={{ color: "#E65100" }} /></div>
                          <span style={{ color: B.mutedFg, fontSize: 12, fontWeight: 600 }}>{crop.location || "—"}</span>
                        </div>
                      </td>
                      <td style={{ padding: "14px 22px", textAlign: "center" }}>
                        <span style={{ fontFamily: "'Space Grotesk', sans-serif", fontSize: 15, fontWeight: 900, color: B.primary, background: B.muted, border: `1.5px solid ${B.border}`, borderRadius: 10, padding: "4px 12px", display: "inline-block" }}>
                          ${crop.price}
                        </span>
                      </td>
                      <td style={{ padding: "14px 22px", textAlign: "center" }}>
                        <div style={{ display: "inline-flex", alignItems: "center", gap: 6, padding: "5px 10px", borderRadius: 20, background: cfg.bg, border: `1.5px solid ${cfg.border}` }}>
                          ...
                          <select value={crop.status || "pending"} onChange={(e) => updateStatus(crop._id, e.target.value)}
                      ...
                      <td style={{ padding: "14px 22px", textAlign: "right" }}>
                        <button className="del-btn" onClick={() => deleteCrop(crop._id)}
                          style={{ display: "inline-flex", alignItems: "center", gap: 6, padding: "7px 14px", borderRadius: 10, background: "linear-gradient(135deg, #C62828, #b71c1c)", color: "#fff", fontSize: 11, fontWeight: 800, border: "none", cursor: "pointer", textTransform: "uppercase", letterSpacing: "0.08em", boxShadow: "0 3px 10px rgba(198,40,40,0.25)" }}>
          ...
          {!loading && filtered.length === 0 && (
            <div style={{ padding: "52px 0", textAlign: "center" }}>
              ...
              <p style={{ color: B.border, fontWeight: 800, fontSize: 11, textTransform: "uppercase", letterSpacing: "0.16em", margin: 0 }}>
                {search ? "No crops match your search" : "No crop listings found"}
              </p>
            </div>
          )}
        </div>
        ...
        <div className="mobile-cards" style={{ flexDirection: "column", gap: 12, animation: "fadeUp 0.5s ease 0.1s both" }}>
          ...
          {!loading && filtered.map((crop) => {
            const cfg = STATUS_CFG[crop.status] || STATUS_CFG.pending;
            const StatusIcon = cfg.icon;
            return (
              <div key={crop._id} className="crop-card" style={{ background: B.card, border: `1.5px solid ${B.border}`, borderRadius: 18, padding: "16px", boxShadow: "0 2px 10px rgba(46,125,50,0.06)", position: "relative", overflow: "hidden" }}>
                ...
                <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 10, marginBottom: 14 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                    <div style={{ padding: "10px 11px", borderRadius: 12, background: B.muted, border: `1.5px solid ${B.border}`, flexShrink: 0 }}>
                      <Sprout size={18} style={{ color: B.primary }} />
                    </div>
                    <div>
                      <div style={{ fontWeight: 800, fontSize: 15, color: B.foreground, lineHeight: 1.2 }}>{crop.title}</div>
                      <div style={{ fontSize: 10, fontWeight: 900, color: B.primary, textTransform: "uppercase", letterSpacing: "0.1em", marginTop: 4, background: B.muted, border: `1px solid ${B.border}`, borderRadius: 6, padding: "1px 7px", display: "inline-block" }}>
                        {crop.cropType}
                      </div>
                    </div>
                  </div>
                  {/* Price badge */}
                  <span style={{ fontFamily: "'Space Grotesk', sans-serif", fontSize: 15, fontWeight: 900, color: B.primary, background: B.muted, border: `1.5px solid ${B.border}`, borderRadius: 10, padding: "5px 12px", flexShrink: 0 }}>
                    ${crop.price}
                  </span>
                </div>

                {/* Row 2: Location */}
                <div style={{ display: "flex", alignItems: "center", gap: 7, marginBottom: 14, padding: "8px 12px", borderRadius: 10, background: "#FBE9E7", border: "1px solid #FFCCBC" }}>
                  <MapPin size={12} style={{ color: "#E65100", flexShrink: 0 }} />
                  <span style={{ color: "#E65100", fontSize: 12, fontWeight: 700 }}>{crop.location || "Location not set"}</span>
                </div>
                ...
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 10, paddingTop: 12, borderTop: `1px solid ${B.border}` }}>
                  <div style={{ display: "inline-flex", alignItems: "center", gap: 6, padding: "7px 12px", borderRadius: 20, background: cfg.bg, border: `1.5px solid ${cfg.border}` }}>
                    <StatusIcon size={12} style={{ color: cfg.color }} />
                    <select value={crop.status || "pending"} onChange={(e) => updateStatus(crop._id, e.target.value)}
                      style={{ border: "none", background: "transparent", outline: "none", color: cfg.color, fontSize: 11, fontWeight: 900, textTransform: "uppercase", letterSpacing: "0.07em", cursor: "pointer" }}>
                      <option value="pending">Pending</option>
                      <option value="approved">Approved</option>
                      <option value="hidden">Hidden</option>
                      <option value="rejected">Rejected</option>
                  ...
                  <button className="del-btn" onClick={() => deleteCrop(crop._id)}
                    style={{ display: "inline-flex", alignItems: "center", gap: 6, padding: "8px 16px", borderRadius: 10, background: "linear-gradient(135deg, #C62828, #b71c1c)", color: "#fff", fontSize: 11, fontWeight: 800, border: "none", cursor: "pointer", textTransform: "uppercase", letterSpacing: "0.08em", boxShadow: "0 3px 10px rgba(198,40,40,0.25)", flexShrink: 0 }}>
                    <Trash2 size={12} /> Remove
                  </button>
                </div>
              </div>
        ...
        <footer style={{ display: "flex", alignItems: "center", justifyContent: "space-between", animation: "fadeUp 0.5s ease 0.2s both" }}>
          <button onClick={() => loadCrops(pagination.page - 1)} disabled={pagination.page <= 1}
          ...
          <button onClick={() => loadCrops(pagination.page + 1)} disabled={pagination.page >= pagination.totalPages}
