...
export async function POST(request) {
  try {
    ...
    const redirectUrl = `${process.env.NEXT_PUBLIC_BASE_URL}/payment-status?status=cancelled&tran_id=${tranId || ""}`;
...
<html>
  ...
  <body><p style="font-family:sans-serif;text-align:center;margin-top:40px">Redirecting...</p></body>
    ...
    const errorUrl = `${process.env.NEXT_PUBLIC_BASE_URL}/payment-status?status=cancelled`;
    return new Response(
...
export async function GET(request) {
  ...
  return NextResponse.redirect(
    `${process.env.NEXT_PUBLIC_BASE_URL}/payment-status?status=cancelled&tran_id=${tranId}`
  );
}