...
export default function Sidebar({ userRole = "admin" }) {
  ...
  const isDashboard = item.href === "/admin" || item.href === "/farmer" || item.href === "/buyer";
  ...
  return (
    ...
    >
      ...
      <span className={`text-[13px] font-bold tracking-tight ${isActive ? "text-white" : "text-slate-600"}`}>
        {item.name}
      </span>
        ...
        <div className="bg-slate-50 border border-slate-100 rounded-2xl p-4 flex flex-col gap-3">
          <div className="flex items-center gap-3">
             ...
             <div className="flex flex-col">
                <span className="text-[11px] font-black text-slate-800 uppercase leading-none">Session Active</span>
                <span className="text-[9px] font-bold text-slate-400 mt-1 truncate w-32">{userRole}@krishinova.com</span>
             </div>
          </div>
